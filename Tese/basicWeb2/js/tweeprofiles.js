...
var len = items.length;	
	...
	for(var i = 0; i < len; i++){
		...
		var circle = new google.maps.Circle({
			center:pos,
			radius:items[i].radius*20200*(items[i].weight/items[i].n)*250,
			strokeColor:"#000000",
			strokeOpacity:0.8,
			strokeWeight:0.75,
			fillColor:"#4099FF",
		...
		google.maps.event.addListener(circle, 'click', function() {
				...
				setTheseMicroClusters(this.clusterid);
  		});
	}
}else{
	var len = items.length;	
...
function setTheseMicroClusters(clusterid){
        ...
        'http://localhost:8080/basicWeb/index.php/site/getMicroIds/'+clusterid+'/'+type+'',
         ...
         function(data) { 
            	items=data;
		        setTheseMicroClusters_2(items, clusterid);
		        // loadTimeline(items);
		        // showClustersGraph(items);
         }, 'JSON');
}

function setTheseMicroClusters_2(mcIds, clusterid){
		var items = [];
		var type = 0;
        ...
        'http://localhost:8080/basicWeb/index.php/site/getMicroClusters',
         ...
         function(data) { 
            	items=data;
		        showTheseMicroClustersMap(items, mcIds, clusterid);
		        // loadTimeline(items);
		        // showClustersGraph(items);
         }, 'JSON');
}
...
function showTheseMicroClustersMap(items, mcIds, clusterid){
